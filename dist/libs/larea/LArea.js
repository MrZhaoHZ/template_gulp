...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        init: function(params) {
            MobileAreaPram = params;
            this.params = params;
            this.trigger = document.querySelector(params.clickDom);
            if(params.valueTo){
        ...
        finish: function(e) {
            ...
            var value = provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):(''));
            var dom = '<textarea readonly onfocus="this.blur()" class="address" id="demo2" type="text" placeholder="选择省市区" >' + value + '</textarea>'
             $(MobileAreaPram.trigger).remove();
            
            $(MobileAreaPram.clickDom).append(dom);
